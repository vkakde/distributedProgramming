...
public class Server extends UnicastRemoteObject
    ...
    private void makeActive (InetSocketAddress s) {
    	System.out.println("\nClient inet address: "+s.toString());
    	clientSocket = s;
    	mode = Mode.ACTIVE;
    }
    		 ...
    		 */
    		 try {
    			 ...
    			 OutputStream os = ClientSocket.getOutputStream();
          		 ...
          		 // initialize byte buffer to read into from file
    	         int bytesRead = 0;
    ...
    public void get (String file) throws IOException, FileNotFoundException, RemoteException {
        if (!valid(file)) {
        	...
        	System.out.println("\nReceived get request [ACTIVE]! + client socket: "+clientSocket.toString()); 
        	...
        	try {
        		...
        		///cite http://www.rgagnon.com/javadetails/java-0542.html
        		...
        		// keep reading until no more to read. Write read bytes to output stream
        		while ((bytesRead = in.read(byteBuffer, 0, byteBuffer.length)) > 0) {
        			os.write(byteBuffer, 0, bytesRead);
        		}
    	...
    	public void run () {
    		 try {
    			 ...
    			 InputStream is = ClientSocket.getInputStream();
          		 ...
          		 // initialize byte buffer to write into file
    	         int bytesRead = 0;
    	         byte[] byteBuffer = new byte[1024];

         		 // keep reading until no more to read. Write read bytes to output file
